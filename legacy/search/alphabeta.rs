...
use crate::titanium::search::cat_index_lmr::cat_index_lmr_reduction;
use crate::legacy_search::lmr_profile::{
    apply_depth_feedback, build_lmr_table, compute_stage_t, EvalZoneState, LmrProfile,
    MateStopReason, MateZoneState,
};
use crate::legacy_search::move_pack::{pack_move, unpack_move};
use crate::legacy_search::search_tt::{SearchTt, TtBound};
...
struct SearchState<'a> {
    config: SearchConfig,
    tt: &'a mut SearchTt,
    killers: &'a mut [[u32; 2]; crate::legacy_search::session::MAX_KILLER_PLY],
    history: &'a mut [i16; crate::legacy_search::session::HISTORY_SIZE],
    pv_hint: Option<Move>,
...
fn update_lmr_profile_for_depth(
    ...
    if state.log && std::env::var("TITANIUM_LOG").is_ok() {
        ...
        let time_p = crate::legacy_search::lmr_profile::time_pressure_from_ms(state.config.time_ms);
        let pierce =
            crate::legacy_search::lmr_profile::LmrProfile::pierce_strength(state.fraction_elapsed());
        eprintln!(
...
fn record_killer(
    killers: &mut [[u32; 2]; crate::legacy_search::session::MAX_KILLER_PLY],
    ply: usize,
    mv: Move,
) {
    if ply >= killers.len() {
...
fn history_bonus(history: &[i16; crate::legacy_search::session::HISTORY_SIZE], mv: Move) -> i32 {
    let packed = pack_move(mv);
    if packed == 0 {
        0
    } else {
...
fn bump_history(history: &mut [i16; crate::legacy_search::session::HISTORY_SIZE], mv: Move, depth: u32) {
    let packed = pack_move(mv);
    if packed == 0 {
        return;
    }
...
fn emit_search_progress(
    ...
    let profile_json =
        crate::legacy_search::lmr_viz::lmr_profile_fields(&state.lmr_profile, state.search_depth);
    eprintln!(
...
fn emit_json_report(report: &SearchReport, profile: &LmrProfile, log: bool) {
    ...
    let profile_json = crate::legacy_search::lmr_viz::lmr_profile_fields(profile, report.search_depth);
    eprintln!(
...
mod tests {
    ...
    fn endgame_loss_never_leaks_phantom_draw() {
        ...
        let mut oracle = crate::titanium::exact_dp::Oracle::default();
