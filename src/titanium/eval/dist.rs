...
/// Bits of all cells in the straight column from `sq` (inclusive) north to row 1.
/// The pawn needs a north step from each of these cells to reach row 0.
/// If `(STRAIGHT_NORTH_BITS[sq] & masks.north) == STRAIGHT_NORTH_BITS[sq]`,
/// the straight path is unblocked and the distance to row 0 is exactly `row(sq)`.
const fn straight_north_bits_table() -> [u128; 81] {
    let mut t = [0u128; 81];
    let mut sq = 0usize;
    while sq < 81 {
        let row = sq / 9;
...
/// Bits of all cells in the straight column from `sq` (inclusive) south to row 7.
/// The pawn needs a south step from each of these cells to reach row 8.
/// If `(STRAIGHT_SOUTH_BITS[sq] & masks.south) == STRAIGHT_SOUTH_BITS[sq]`,
/// the straight path is unblocked and the distance to row 8 is exactly `8 - row(sq)`.
const fn straight_south_bits_table() -> [u128; 81] {
    let mut t = [0u128; 81];
    let mut sq = 0usize;
    while sq < 81 {
        let row = sq / 9;
...
const STRAIGHT_NORTH_BITS: [u128; 81] = straight_north_bits_table();
const STRAIGHT_SOUTH_BITS: [u128; 81] = straight_south_bits_table();

/// If the pawn's straight column to row 0 is unblocked, return `Some(row)` — the
/// exact distance with no flood. Correct because a straight north path only crosses
/// north/south edges, so only horizontal walls can invalidate it.
#[inline]
pub fn pawn_dist_straight_to_row0(pawn_sq: usize, masks: DirMasks) -> Option<u8> {
    let path = STRAIGHT_NORTH_BITS[pawn_sq];
    if (path & masks.north) == path {
        return Some((pawn_sq / 9) as u8);
    }
    None
}

/// If the pawn's straight column to row 8 is unblocked, return `Some(8 - row)`.
#[inline]
pub fn pawn_dist_straight_to_row8(pawn_sq: usize, masks: DirMasks) -> Option<u8> {
    let path = STRAIGHT_SOUTH_BITS[pawn_sq];
    if (path & masks.south) == path {
        return Some(8 - (pawn_sq / 9) as u8);
    }
    None
}

/// Convenience: straight-line distance to `player`'s own goal row.
#[inline]
pub fn pawn_dist_straight(pawn_sq: usize, player: usize, masks: DirMasks) -> Option<u8> {
    if player == 0 {
        pawn_dist_straight_to_row0(pawn_sq, masks)
    } else {
        pawn_dist_straight_to_row8(pawn_sq, masks)
    }
}
