//! Perft node cache.
//!
//! Relocated out of the legacy search tree, where it happened to live but did
//! not belong: nothing but perft has ever used it (its own header called it
//! "perft node cache"). Keeping it here is what let the legacy engine be
//! removed without taking movegen validation with it.
...
pub struct WorkerContext {
    pub bfs: crate::pathfinding::BfsScratch,
}
...
impl WorkerContext {
    pub fn new() -> Self {
        Self {
            bfs: crate::pathfinding::BfsScratch::new(),
        }
    }
}
